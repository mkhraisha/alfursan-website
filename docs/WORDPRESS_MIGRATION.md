...
- [x] **One-time image migration: WP media → `vehicle-images` bucket**
  - **Description:** `scripts/migrate-wordpress-photos.mjs` fetches every WP `cars` post's `vehica_6673` image URLs, and for each DMS vehicle whose `images_json` is still empty (`planVehiclePhotoMigration` in `src/lib/wordpress-photo-migration.ts`), downloads each URL (resolved to `media.alfursanauto.ca` via `toMediaUrl`) and uploads it into the Supabase `vehicle-images` bucket at `vehicles/{vin}/wp-NN.{ext}` — same bucket and `vehicles/{vin}/...` prefix convention `upload-url.ts` uses for admin-uploaded photos. `images_json` is then set to the uploaded paths in WP order (index 0 = featured, matching the existing admin convention), and `photography_status` flips from `pending`/unset to `done` (`buildPhotographyStatusPatch`) so the vehicle becomes eligible for Part 3's visibility rule — but only then; a staff-set `na` or already-`done` value is never overridden. A vehicle whose `images_json` is already populated is skipped entirely and never re-migrated, since photo uploads happen only through the DMS going forward (decision 4) and an admin may have already curated photos since Part 2 ran. A vehicle with no WP images at all is also skipped (nothing to migrate). Partial failures (some images download/upload, others don't) still write whatever succeeded — a vehicle only fails outright if every one of its images fails. Same `--dry-run`/`--allow-production` local-only safety guard as Part 2's script, plus `--vin=X` to spot-check a single vehicle and `--concurrency=N` (default 4) to bound parallel downloads/uploads.
  - **Validation:** Every migrated vehicle's `images_json` resolves to a working public URL via `buildStorageUrl(supabaseUrl, "vehicle-images", path)` — same helper the admin Media tab already uses. Confirmed directly: ran the script in write mode against a local Supabase stack for a real WP vehicle (15 images), all 15 uploaded successfully, `images_json` populated in order, and the first path's public storage URL returned `200 image/jpeg` with real image bytes. Re-ran the same vehicle a second time — 0 uploaded, skipped with reason "images_json already populated" (idempotent, no duplicate uploads). `--dry-run` against live WordPress: 43 cars fetched, 37 candidates with a DMS-matching VIN and photos, 373 images total, 0 candidates missing from the DMS.
  - **Test:** `src/__tests__/wordpress-photo-migration.test.ts` covers the pure planning/decision logic (21 tests): media-URL rewriting, extension detection/fallback, ordered storage-path construction, skip-if-already-populated / skip-if-no-images, URL dedup, and the photography_status patch rules (done-on-pending, done-on-null, never-overrides-done-or-na).
