...
**Status: script built and validated in dry-run against live WordPress data — see PR for `feat/wp-migration-part2-data-import`. The actual write-mode run against a real local/staging Supabase stack (and the human sign-off on the reconciliation report) still needs to happen before cutover — see the open item below.**

- [x] **One-time import script: WP `cars` → `vehicles`**
  - **Description:** `scripts/migrate-wordpress-inventory.mjs` fetches all WP `cars` posts + taxonomy terms, maps each field via the pure functions in `src/lib/wordpress-migration.ts` onto the schema added in Part 1, and inserts into `vehicles` by VIN (write mode only — `--dry-run` never touches Supabase). Vehicles with a missing/invalid VIN, make, model, year, or body type are reported and skipped, not silently dropped. `images_json`/`videos_json`/`photography_status` are deliberately left untouched — Part 4 migrates photos separately, and a vehicle with no photos yet should stay non-public under Part 3's visibility rule.
  - **Validation:** Every mapped row is re-validated against `vehicleCreateSchema` before insert as a safety net. Skipped rows are written to `skipped.json` with a reason; existing-VIN collisions are detected before insert and reported, never overwritten.
  - **Test:** `src/__tests__/wordpress-migration.test.ts` covers the pure mapping/validation logic (31 tests). `--dry-run` was run against the live WordPress site: 43 cars fetched, 36 would migrate, 7 skipped (6 missing VIN, 1 missing model — real WP data-quality gaps, not bugs). Actual write-mode insert against a local Supabase stack still needs to happen (see below) — this environment's shared local stack made that unsafe to test here (see [[project_wordpress_migration_parallel_agent]]-style multi-agent caution: other sessions may have state in the same local stack).
...
- [x] **Reconciliation report**
  - **Description:** The script writes `report.md`/`report.json` (counts, warnings, insert errors), `skipped.json` (every skipped row with its reason), and `slug-to-vin.json` (old WP slug → VIN, feeding Part 5's redirect table) to `docs/migration/wordpress-inventory/<timestamp>/`.
  - **Validation:** Reviewed in dry-run above. **Open item:** the real write-mode run (against a real local/staging Supabase stack, not this shared dev environment) and human sign-off on VIN collisions still need to happen before cutover — do this right before Part 5 goes live, not far in advance, so the data stays fresh.
