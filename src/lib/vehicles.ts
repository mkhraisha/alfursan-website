...
export const vehicleBaseSchema = z.object({
  ...
  drive_type:       z.enum(DRIVE_TYPES).optional().nullable(),
  transmission:     z.enum(TRANSMISSIONS).optional().nullable(),
  fuel_type:        z.enum(FUEL_TYPES).optional().nullable(),
  cylinders:        z.number().int().min(1).optional().nullable(),
  ...
  advertised_price_cargurus: positiveDecimal.optional().nullable(),
  advertised_price_facebook: positiveDecimal.optional().nullable(),
  sale_price:       positiveDecimal.nullable().optional(),
  sale_date:        isoDate.nullable().optional(),
  ownership_status: z.enum(["available", "en_route", "not_received"]).optional().nullable(),
  status:           z.enum(VALID_STATUSES).nullable().optional(),
  photography_status: z.enum(["pending", "done", "na"]).optional().nullable(),
