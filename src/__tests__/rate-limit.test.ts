...
import { getFinancingRateLimit, getDescriptionRateLimit } from "../lib/rate-limit";
...
describe("getDescriptionRateLimit", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.clearAllMocks();
  });

  it("throws when Upstash env vars are missing", () => {
    vi.stubEnv("UPSTASH_REDIS_REST_URL", "");
    vi.stubEnv("UPSTASH_REDIS_REST_TOKEN", "");
    expect(() => getDescriptionRateLimit()).toThrow(
      "Missing UPSTASH_REDIS_REST_URL or UPSTASH_REDIS_REST_TOKEN"
    );
  });
  ...
  it("constructs Ratelimit with its own prefix and a tighter sliding window than financing", () => {
    vi.stubEnv("UPSTASH_REDIS_REST_URL", "https://redis.upstash.io");
    vi.stubEnv("UPSTASH_REDIS_REST_TOKEN", "tok_secret");
    getDescriptionRateLimit();
    expect(Ratelimit).toHaveBeenCalledWith(
