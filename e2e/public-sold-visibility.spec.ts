 ...
 * days ago must not be.
 *
 * Seeds two dedicated vehicles directly against the local Supabase stack
...
test.describe("Public visibility of sold vehicles", () => {
  ...
  test.beforeAll(async () => {
    ...
    const { error: cleanupError } = await admin
      .from("vehicles")
      .delete()
      .in("vin", [RECENT_SOLD_VIN, STALE_SOLD_VIN]);
  ...
  test.afterAll(async () => {
    await admin.from("vehicles").delete().in("vin", [RECENT_SOLD_VIN, STALE_SOLD_VIN]);
  });
  ...
  test("/search/ renders a vehicle sold within the last 30 days, but not one sold over 30 days ago", async ({ page }) => {
    const response = await page.goto("/search/");
    expect(response?.status()).toBe(200);

    await expect(page.getByRole("link", { name: /E2eRecentSold/ }).first()).toBeVisible({ timeout: 10_000 });
    await expect(page.getByRole("link", { name: /E2eStaleSold/ })).toHaveCount(0);
  });

  test("homepage renders a vehicle sold within the last 30 days, but not one sold over 30 days ago", async ({ page }) => {
